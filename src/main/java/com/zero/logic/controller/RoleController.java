...
public class RoleController {
        ...
        try {
            ...
            for (String roleId : roleIds) {
                ...
                if (null != role && role.getState() == 0) {
                    ...
                    unDeleteId += role.getRoleName() + ",";
                }
            }
            if ("".equals(unDeleteId)) {
                return JsonUtil.returnStr(JsonUtil.RESULT_SUCCESS, "删除角色成功");
            } else {
                unDeleteId = unDeleteId.substring(0, unDeleteId.length() - 1);
    ...
    public String changeUserState(
        ...
        try {
            ...
            return JsonUtil.returnStr(JsonUtil.RESULT_SUCCESS, "角色修改成功");
        } catch (Exception e) {
            return JsonUtil.returnStr(JsonUtil.RESULT_SUCCESS, "角色修改失败");
        }
    }
}
